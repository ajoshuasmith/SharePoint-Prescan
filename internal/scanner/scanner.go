...
func (s *Scanner) isHidden(name, path string) bool {
	...
	if runtime.GOOS == "windows" {
		return s.isHiddenWindows(path)
	}

	return false
}

func (s *Scanner) isHiddenWindows(path string) bool {
	...
	// This is a simplified check - full implementation would use syscall
	return info.Mode()&os.ModeType != 0
}

func (s *Scanner) isSystem(path string, info os.FileInfo) bool {
	// System files typically have special attributes on Windows
	if runtime.GOOS == "windows" {
		// Simplified check - full implementation would check FILE_ATTRIBUTE_SYSTEM
		return false
	}
	return false
}
